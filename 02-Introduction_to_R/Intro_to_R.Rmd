...
One way to find out whether a linear model is a correct fit or not is to actually plot the residuals.
...
abline (0,0, col="red")
```
**Homoscedasticiy** is when residuals are equally distributed above and below the horizontal line. **Heteroscedasticity** is there is a bias. For this time looks like scedasticity is pretty good.

The other thing you should check is the whether the residuals are distributed normally.

```{r}
hist (res)
```

In this case it fit does not look very good. However, our dataset is pretty small. The normality checking should be reliable only for larger dataset.

## Generalized linear model

The test for comparing means between two groups is `t.test`. The test for comparing means among two or more groups is `anova`. And we have already seen linear regression. All these are just subclasses of *Generalized Linear Model*. We will not discuss this in details in this course. However, just to show that are all the same see the following code.

First we will do the t.test using iris dataset. 

```{r}
data("iris")
iris$Species <- as.character(iris$Species)
two_species_iris <- iris[iris$Species == "setosa" | iris$Species=="virginica",]
```


```{r}
t.test(Petal.Length~Species, data=two_species_iris)
```

Note the p-vale above.

```{r}
summary(aov(Petal.Length~Species, data=two_species_iris))
```

P-value is exactly the same.

```{r}
summary(lm(Petal.Length~Species,data=two_species_iris))
```

A through introduction to GLM is beyond the scope of of this course. But to show that regression is just and variation of GLM we can use `glm` function instead of `lm`:

```{r}
summary(glm(women$weight~women$height, family = gaussian))
```

It is identical to the `lm` result found before.
...
summary(petal_model)
predict(petal_model, data.frame(Petal.Width=c(0.7)))
```
